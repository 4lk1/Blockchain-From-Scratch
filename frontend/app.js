...
class UIRenderer {
    ...
    async renderStats() {
        try {
            ...
            if (navDifficulty) navDifficulty.textContent = stats.mining_difficulty || stats.difficulty || 3;
            ...
            if (integrity) integrity.textContent = stats.chain_valid ? 'Valid ✓' : 'Invalid ✗';

            // Update chain validity indicator color
            if (chainValid) {
                chainValid.style.color = stats.chain_valid ? 'var(--accent-success)' : 'var(--accent-danger)';
            }
        } catch (err) {
    ...
    async renderMiningStats() {
        try {
            ...
            if (diffEl) diffEl.textContent = stats.mining_difficulty || stats.difficulty || 3;
            if (rewardEl) rewardEl.textContent = stats.mining_reward ?? '10.0';
            if (valueEl) {
                valueEl.textContent = stats.total_value ?? 0;
            }
        } catch (err) {
    ...
    setupEventListeners() {
        ...
        window.submitTransaction = async () => {
            ...
            try {
                ...
                const senderName = this.renderer.state.wallets.find(w => w.address === sender)?.name || sender.substring(0,10);
                const receiverName = this.renderer.state.wallets.find(w => w.address === receiver)?.name || receiver.substring(0,10);
...
UIRenderer.prototype.renderCurrentStats = async function() {
    try {
        ...
        const elements = {
            ...
            'chain-valid': stats.chain_valid ? 'Valid ✓' : 'Invalid ✗'
        };
