...
class WalletService:
    ...
    def create_wallet(self, name: str) -> Dict:
        ...
        return {
            "address": address,
            "name": name,
            "public_key": wallet.get_public_key().decode() if wallet.get_public_key() else None,
            "balance": 0.0,
        }
        ...
        for address, nm in self.wallet_names.items():
            if nm == name:
                ...
                return {
                    ...
                    "name": nm,
                    "balance": self.wallet_balances.get(address, 0.0),
                    "public_key": wallet.get_public_key().decode() if wallet and wallet.get_public_key() else None,
                }
        return None
...
class TransactionService:
    ...
    def create_transaction(
        ...
        # Check sufficient balance using WalletService balances
        sender_balance = self.wallet_service.get_wallet_balance(sender_address)
        if sender_balance < amount:
            raise ValueError(
                f"Insufficient balance. Have {sender_balance}, need {amount}"
            )
...
class MiningService:
    ...
    def mine_block(self, miner_address: str) -> Dict:
        ...
        # Update miner wallet balance using WalletService
        current_balance = self.wallet_service.get_wallet_balance(miner_address)
        self.wallet_service.update_wallet_balance(
            miner_address, current_balance + blockchain.mining_reward
        )
