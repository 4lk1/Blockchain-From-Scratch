...
class Blockchain:
    ...
    def add_transaction(self, transaction):
        ...
        if not transaction.is_valid():
            print(f"Transaction rejected: Invalid signature or unsigned")
            return False
        
        self.pending_transactions.append(transaction)
        print(f"✓ Transaction added: {transaction.sender[:10]}...→{transaction.receiver[:10]}... ({transaction.amount})")
        return True
        ...
        for i in range(len(self.chain)):
            ...
            if i > 0:
                if current_block.previous_hash != self.chain[i-1].hash:
                    error_msg = f"Block #{i}: Previous hash mismatch\n   Expected: {self.chain[i-1].hash}\n   Got: {current_block.previous_hash}"
                    print(error_msg)
                    return False, error_msg
            
            # Check Proof-of-Work
            if not current_block.hash.startswith('0' * self.difficulty):
                error_msg = f"Block #{i}: Invalid Proof-of-Work (insufficient leading zeros)"
                print(error_msg)
                return False, error_msg
    ...
    def print_chain(self):
        ...
        for block in self.chain:
            ...
            for i, tx in enumerate(block.transactions):
                print(f"    [{i}] {tx.sender[:10]}... → {tx.receiver[:10]}... : {tx.amount}")
